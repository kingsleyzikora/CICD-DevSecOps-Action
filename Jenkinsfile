...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit kingsleyzikora/reddit:latest "
                       sh "docker push kingsleyzikora/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image kingsleyzikora/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.144.170.52  "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.134.93.204:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#zikky-devsecops', //update and provide your channel name
